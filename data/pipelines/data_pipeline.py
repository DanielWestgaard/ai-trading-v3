...
class DataPipeline:
    ...
    def run(self, source=None, target_path=sys_config.CAPCOM_PROCESSED_DATA_DIR, 
        ...
        # 3. Generate features - this is the new part
        self.feature_generator.fit(clean_data)
        featured_data = self.feature_generator.transform(clean_data)
        
        # 4. Normalize - includes all features now
        self.normalizer.fit(featured_data)
        normalized_data = self.normalizer.transform(featured_data)
        
        # 5. Save processed data - save the fully normalized & featured data
        if target_path:
            data_utils.save_data_file(normalized_data, "processed", "testing.csv")
